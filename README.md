...
사진을 열고 사람·사물·텍스트·장소를 선택한 뒤 **이 대상으로 사진 찾기**를 누르면 관련 사진을 찾습니다. 도착한 사진에서는 주변 맥락을 보고 새로운 관심사를 따라 탐색을 이어갑니다.

## 주요 기능

| 기능 | 사용 방법과 동작 |
|---|---|
| 홈 / 날짜별 | 홈에서는 자유로운 사진 배치를, 날짜별 탭에서는 최신 촬영일순 정사각 격자를 봅니다. 각 탭의 스크롤 위치를 유지합니다. |
| 사진 연결 | 접근을 허용한 사진 최대 1,000장을 PC 서버에 연결합니다. 새로고침으로 전송·분석을 재개합니다. |
| Connect | 사진 속 대상을 선택하고 **이 대상으로 사진 찾기**를 누릅니다. 길게 눌러 수동 영역을 선택할 수도 있습니다. |
...
![홈, 날짜별, 대상 선택, 연결 결과, 자동 맥락 화면](docs/images/android-paper-ink-2026-09-11.png)

따뜻한 아이보리와 차콜, 앱에 포함한 SUIT 한글 폰트를 사용합니다. Pinterest식 홈과 날짜별 정렬을 분리하고, 선택 영역·연결 결과·자동 맥락에 짧은 애니메이션을 적용했습니다. 위 이미지는 합성 사진과 고정 응답을 사용한 실기기 UI 검수 화면입니다. [최신 구현과 실기기 검증 기록](docs/android-paper-ink-2026-09-11.md)을 참고하세요.
...
앱의 **사진 연결**에서 사진 접근을 허용합니다. 서버를 수동으로 설정하려면 **상단 설정 아이콘 → 서버 연결 → 서버 주소 / 접속 키 → 확인 후 저장**을 사용합니다. 접속 키는 Android Keystore로 암호화해 보관합니다.

디버그 APK에는 캡처·문서·일상·여행 샘플 24장이 포함됩니다. Android 10 이상에서 **설정 → 샘플 사진 추가**를 누르면 기기 갤러리에 넣어 볼 수 있습니다. 빈 갤러리에서는 **샘플 사진으로 시작하기**로도 접근할 수 있습니다. 반복 추가 시 기존 샘플을 유지합니다. [샘플 구성과 검증](docs/sample-gallery.md)을 참고하세요.
...
- [Android 디자인·최신 APK 검증](docs/android-design-2026-09-10.md)
- [앱 아이콘과 브랜드 자산](assets/branding/README.md)
