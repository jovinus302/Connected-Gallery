...
See → Tap → Follow → Tap → Follow. Organize는 실제 Gallery의 재사용 맥락 진입점이며 Connect가 핵심이다. Time은 선택한 의미를 유지하는 modifier다. Rediscovery는 결과다.
...
상태는 SemanticAnchor / FollowDirection / TimeSelection을 분리한다. 새 요소 탭은 새 anchor와 hop, Related·전체 기간으로 초기화한다. 결과 사진 열기는 기존 맥락 유지. 연도와 방향 변경은 anchor 유지. 뒤로 가기는 사진·맥락·기간·스크롤 복원이다.

Same moment는 기준 사진의 사건 맥락으로 확장한다. 대상이 모든 결과에 등장할 필요는 없고 고정 시간창으로 사건을 확정하지 않는다. 연도는 검색 전 강제 적용하며 결과 저장 시 재검증한다. 촬영일 불명 사진은 확정 연도 결과에서 제외한다.
...
입력 사진·OCR·caption은 신뢰하지 않는 데이터다. 이름·관계·ID를 추측하지 않는다. 데이터 검증·실행 예산·좌표 변환·명시적 연도 제약은 결정적인 코드로 보장한다.
...
분석 동시 3, GPU 도구 1, 사용자 탐색 우선. 모델 턴 상한 analyst4/explorer6/organizer30, 도구12/12/100. Explorer45초. 일시 호출 실패1회 재시도와 대체 모델1회 후 실패·부분완료 표시.

## 성능 목표 (실측 전)

사진·탭 p95 300ms, 캐시500ms, 새 탐색 첫 확인결과5초/최종15초/상한45초. 최초20장3분, 1,000장90분 목표. 모델 준비 시간 별도. 실제 hop 대기와 이탈을 함께 측정한다.

## 10일 일정

D1 계약·AgentRunner·Proxy / D2 Android 뷰어·상태 / D3 동기화·모델·50장 / D4 실제 탭→결과 / D5 연속3hop / D6 방향·Time·기록 / D7 Spaces / D8 캐시·취소·삭제·복구 / D9 500~1,000장 성능·품질 / D10 사용자 평가·APK·문서.

## 평가

50장 개발 세트와 분리된 평가 사진을 사용한다. 가족·문서·구매후보·차량·여행·스크린샷, 다년도·유사 인물·작은 한글·회전·누락 시간·중복·무정답 포함. 합성 fixture는 실제 평가와 분리한다.

Find 성공률80%/중앙값30초/기존 갤러리 대비25% 단축, eligible 2초 이상 사진의 tap30%, 새 요소 hop중앙값2, 재사용4점이상70%, P@5사물·텍스트.8/인물.9 목표. back/time은 hop 제외. 평가 n과 실패를 보고한다.
