...
사진 상세 맥락의 후보 확보·입력·API·준비·화면 상태는 [계약](photo-context-contract.md)과 [통합 검증](context-ux-validation.md)으로 갱신했다. 아래의 Space 진입점, Same moment 동작, 전역 Organizer와 초기 완료 조건은 당시 계획 이력이며 최신 UX의 완료 여부를 판단하는 기준으로 사용하지 않는다.
