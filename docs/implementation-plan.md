...
사진 상세 맥락의 후보 확보·입력·API·사전 준비·화면 상태는 PC 데모에 구현하고 검증했다. [2026-09-08 PC 데모 완료 기록](pc-demo-completion.md)에 현재 준비율·검증 범위·실행 방법을 정리했다. 아래의 Space 진입점, Same moment 동작, 전역 Organizer와 초기 완료 조건은 당시 계획 이력이며 최신 UX의 완료 여부를 판단하는 기준으로 사용하지 않는다.
...
MVP의 Spaces 노출·모으기·Android 호출과 모듈, 서버 목록·편집·새 Organizer 실행·자동 생성 경로를 제거 또는 차단했다. 기존 대기 작업의 재시작도 차단하며 Space 데이터는 보존한다. [Space 제거 검증](space-removal-validation.md)에 실제 범위를 기록한다. 사진 상세의 자동 Organize 맥락과 Android 흐름을 통합했으며 실제 사진 품질·성능의 남은 항목은 통합 검증에 기록한다.

PC 데모와 최신 main의 통합에서는 Context v6 계약을 사용한다. [머지 검증과 이전 데이터 처리](demo-main-integration.md)를 확인한다.
