...
class ContextService:
    ...
    def key(self, photo_id):
        ...
        return hashlib.sha256(encoded(["photo-context", CONTEXT_SPEC, CONTEXT_POLICY, CONTEXT_WORDING_MODEL,
            os.getenv("CG_MODEL", "gpt-5.4-mini"), photo_id, asset.version, self.store.revision]).encode()).hexdigest()
    ...
    def start(self, photo_id):
        with self.store.lock:
            ...
            request = RunRequest(role="context", photo_ids=[photo_id],
                idempotency_key=f"context:{self.key(photo_id)}:{rid}")
            cached = self.store.cache_get(self.key(photo_id)) if value["state"] in ("ready", "empty") else None
